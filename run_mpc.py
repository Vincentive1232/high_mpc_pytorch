...
def arg_parser():
    ...
    parser.add_argument('--save_video', type=bool, default=False, help="Save the animation as a video file")

    return parser


def run_mpc(env: Dynamic_Gap):
    env.reset()
    t = 0
    n = 0
    t0 = time.time()
...
def main():
    ...
    so_path = "./mpc/saved/mpc_v1.so"     # saved mpc model generated by casadi

    mpc = MPC(T=plan_T, dt=plan_dt, so_path=so_path)
    env = Dynamic_Gap(mpc, plan_T, plan_dt)
